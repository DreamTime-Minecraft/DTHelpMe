        ...
        if(args.length == 0) {
            ...
            if(args[0].equalsIgnoreCase("2")) {
                ...
                if (s instanceof ProxiedPlayer) {
                    ...
                    TextComponent ref = new TextComponent(TextComponent.fromLegacyText("§a > §7/ref - Увидеть список подкоманд по реферальной системе"));
                    ref.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("§6Нажми, чтобы ввести эту команду!")));
                    ref.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/ref"));
                    s.sendMessage(ref);

                    TextComponent report = new TextComponent(TextComponent.fromLegacyText("§a > §7/report - жалобы на игроков"));
                    report.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, TextComponent.fromLegacyText("§6Нажми, чтобы ввести эту команду!")));
                    report.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/report"));
                    s.sendMessage(report);
                } else {
                    s.sendMessage(TextComponent.fromLegacyText("§cДальше команды только для игроков с:"));
                }
            } else {
                s.sendMessage(TextComponent.fromLegacyText("§cТакой страницы нет."));
            }
        }
    }
}
